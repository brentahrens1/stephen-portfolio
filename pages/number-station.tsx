export default function numberStation() {
    return (
        <div className="page">
            <div className="page__banner">
                <img src="/images/projects/number-station/number-station-main.png" />
            </div>
            <div className="page__content">
                <h2>Context</h2>
                <p>A new musical release by Brooklyn Rap Duo Young Planet, “Dog Days” is a 3 song EP that feels like a hot summer's day. Bouncy production and smooth lyrics paint pictures of trips to the beach, walking on a city street in 90 degree weather with a friend and falling in love.</p>
            </div>
            <div className="page__banner">
                <img src="/images/projects/number-station/number-station-grid.png" />
            </div>
            <div className="page__content">
                <p>A new musical release by Brooklyn Rap Duo Young Planet, “Dog Days” is a 3 song EP that feels like a hot summer's day. Bouncy production and smooth lyrics paint pictures of trips to the beach, walking on a city street in 90 degree weather with a friend and falling in love.</p>
                <h2>The Logo</h2>
            </div>
            <div className="page__image">
                <img src="/images/projects/number-station/number-station-2.png" />
            </div>
            <div className="page__image last">
                <img src="/images/projects/number-station/number-station-3.png" />
            </div>
            <div className="page__content">
                <h2>Context</h2>
                <p>A new musical release by Brooklyn Rap Duo Young Planet, “Dog Days” is a 3 song EP that feels like a hot summer's day. Bouncy production and smooth lyrics paint pictures of trips to the beach, walking on a city street in 90 degree weather with a friend and falling in love.</p>
            </div>
            <div className="page__banner">
                <img src="/images/projects/number-station/number-station-4.png" />
            </div>
        </div>
    )
}